...
from articles_api.settings.production import DEFAULT_FROM_EMAIL
...
class FollowAPIView(APIView):
    def post(self, request, user_id, format=None):
        try:
            ...
            message = f"Hi there, {profile.user.first_name}!!, the user {user_profile.user.first_name}\
                         {user_profile.user.last_name} now follows you"
        ...
        if not user_profile.check_following(profile):
            formatted_response = {
                ...
                "message": f"You can't unfollow {profile.user.first_name} {profile.user.last_name}, \
                    since you were not following them in the first place.",
            }
